...
- `raspberry-conf` 里没有提交到 Git、但恢复时必须有的运行态/私有态文件：
  - `$RASPBERRY_CONF_DIR/.env`
  - `$RASPBERRY_CONF_DIR/ha/config/backups`
  - `$RASPBERRY_CONF_DIR/mosquitto/data`
  - `$RASPBERRY_CONF_DIR/zigbee2mqtt/data` 中的协调器备份、设备数据库、运行状态和外部转换器
- `clash-meta` 的 compose 和主配置
- `caddy/config`
- 当前自定义 systemd service、docker proxy drop-in、cron、`/usr/local/bin` 下的自定义脚本
- ddns-go 配置

已经提交在 Git 里的静态文件，例如 `raspberry-conf/docker-compose.yml`、`README.md`、`.env.example`、基础服务配置，不再作为备份重点；恢复时直接从 Git clone 回来。

默认 `RASPBERRY_CONF_DIR="$HOME/raspberry-conf"`。如果本地验证用的是别的 clone 路径，可以临时覆盖：

```bash
RASPBERRY_CONF_DIR=~/code/github/raspberry-conf ./scripts/02-backup-pi-local.sh
```
